		...
		<div className="product">
			<div className="img-content">
				<div className="product-img">
					<FaChevronLeft color='#bebebe' className="back-arrow" onClick={prevSlide} />
					{imgs.map((img, index) => 
						<div className={index === current ? "slide active" : "slide"} key={index}>
							{index === current && (
								<img className="image" src={urlFor(imgs[index])} key={img._key} alt="image"/>
							)}
						</div>
					)}
				...
				<div className="product-detail">
					...
					<div className="color">
						{colors.map((color, index) => 
							<label key={index}>
								<input type="radio" name="color" key={index} id="color" value={color.productColor} onChange={e => setColor(e.target.value)} />
								<img src={urlFor(color.productColorImg)} alt="" onClick={e => setImage(color.productColorImg)} />
							</label>
						)}
						...
						<div className="sizes">
							{
								sizes.map((size, index) => 
									<label key={index}>
										<input type="radio" name="size" key={index} id="size" value={size} onChange={e => setSize(e.target.value)} />
