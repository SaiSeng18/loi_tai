...
const Receipt = ({ data, pass, setPass }) => {
	...
	const orderData = cart.map(
		(cart) => `${cart.name} [${cart.color}, ${cart.size}, ${cart.quantity}] \n`
	);
			...
			{!formSubmitted ? (
				<div className="container">
					<div className="receipt-header">
						<div className="logo">
							<Image src={images.logomark} />
						</div>
						<div className="logotrademark">
							<Image src={images.loitai} />
						</div>
					...
					<table>
						<tbody>
							...
							<tr className="order">
								<td className="col-1">
									Order<span className="colon">:</span>
								</td>
								<td className="col-2">
									{cart.map((cart, index) => (
										<span key={index}>
							...
							<tr>
								<td className="col-1">
									PRICE<span className="colon">:</span>
								</td>
								<td className="col-2">{`${getTotalPrice()} MMK`}</td>
							</tr>
							<tr>
								<td className="col-1">
									PHONE NUMBER<span className="colon">:</span>
								</td>
								<td className="col-2">{data.ph_no}</td>
							</tr>
							<tr>
								<td className="col-1">
									EMAIL<span className="colon">:</span>
								</td>
					...
					<div className="term">
						<p>
							ALL SALES ARE FINAL. MEANING PURCHASED ITEMS CANNOT BE REFUNDED OR
							RETURNED.
							<br />
				...
				<div className="success">
					<h3>Your order has been made. Thank you for purchasing.</h3>
				</div>
			)}
		</div>
	);
